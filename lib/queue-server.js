...
const MTALocker = require('./mta-locker');
        ...
        this.server.on('client', client => {
            ...
            client.onData = (data, next) => {
                ...
                switch (data.cmd) {
                    ...
                    case 'LOCKMTA':
                        this.mtaLocker.lock(data.pid, data.key);
                        client.send({
                            req: data.req,
                            response: true
                        });
                        break;
                    case 'RELEASEMTA':
                        if(data.pid && !data.key) {
                            this.mtaLocker.clearPidCounters(data.pid);
                        } else {
                            this.mtaLocker.release(data.pid, data.key);
                        }
                        client.send({
                            req: data.req,
                            response: true
