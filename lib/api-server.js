...
class APIServer {
    ...
    setupRoutes() {
        ...
        // search queued mails by field
        this.server.get('/queued/search/:field/:value', (req, res, next) => {
            if (!this.queue) {
                res.json(500, {
                    error: 'Service not yet started'
                });
            ...
            let query = {
                [field]: new RegExp(req.params.value, "i")
            };

            this.queue.mongodb
                .collection(config.queue.collection)
                .find(query)
                .toArray((err, item) => {
                    if (err) {
                        res.json(500, {
        ...
        this.server.get('/locks', (req, res, next) => {
            res.end(JSON.stringify(this.queueServer.mtaLocker.counters));
            next();
        });
    }
}
