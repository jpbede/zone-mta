        ...
        let tryConnect = () => {
            ...
            mxOptions.connectHook = (opts, connOpts, cb) => {
                if (opts.mx.length > 0) {
                    ...
                    this.sendCommand({cmd: 'ISFREEMTA', key: lockKey, mx: opts.mx[0].exchange}, (err, data) => {
                        if (!data.isFree) {
                            ignoreMXHosts.push(connOpts.host); // Ignoring for the next try
                            let err = new Error("mta locked");
                            err.category = "locked";
                            log.info(this.logName, '%s.%s CONNLIMIT domain=%s mx%s', delivery.id, delivery.seq, delivery.domain, opts.mx[0].exchange);
                            //cb(err);
                        } else {
                            // MX is free, so lock it as used and run plugin hooks
                            this.sendCommand({ cmd: 'LOCKMTA', pid: process.pid, key: lockKey}, () => {
            ...
            mxConnect(mxOptions, (err, mx) => {
                if (err && err.category != "locked") {
                    // a error occurred, release it and then return back
                    let lockKey = [connection.options.host, delivery.zoneAddress.address].join(":");
                    this.sendCommand({ cmd: 'RELEASEMTA', pid: process.pid, key: lockKey }, () => {
