                    ...
                    this.getConnectionWithCache(delivery, (err, connection) => {
                        ...
                        if (!connection.http) {
                            ...
                            connection.once('end', () => {
                                ...
                                let lockKey = [connection.options.host, delivery.zoneAddress.address].join(":");
                                this.sendCommand({ cmd: 'RELEASEMTA', pid: process.pid, key: lockKey }, () => false);
        ...
        let tryConnect = () => {
            ...
            mxOptions.connectHook = (opts, connOpts, cb) => {
                if (opts.mx.length > 0) {
                    ...
                    let usedMx = "";
                    opts.mx.forEach((tmpMx) => {
                        if (tmpMx.A.includes(connOpts.host) || tmpMx.AAAA.includes(connOpts.host)) {
                            usedMx = tmpMx.exchange;
                            return;
                        }
                    });
                    this.sendCommand({cmd: 'ISFREEMTA', key: lockKey, mx: usedMx}, (err, data) => {
                        if (!data.isFree && config.mxConfig.connectionLimiter) {
                            log.info(this.logName, '%s.%s CONNLIMIT domain=%s mx=%s', delivery.id, delivery.seq, delivery.domain, usedMx);
                            ignoreMXHosts.push(connOpts.host); // Ignoring for the next try
                            let err = new Error("mta locked");
                            err.category = "locked";
                            cb(err);
                        } else {
                            // MX is free, so lock it as used and run plugin hooks
            ...
            mxOptions.connectError = (err, opts, connOpts) => {
                ...
                // can't connect, release it
                let lockKey = [connOpts.host, connOpts.localAddress].join(":");
                this.sendCommand({ cmd: 'RELEASEMTA', pid: process.pid, key: lockKey }, () => false);
            };

            mxConnect(mxOptions, (err, mx) => {
                if (err && err.category != "locked") {
                    ...
                    callback(err);
                } else if (err && err.category == "locked") {  // MTA has already a lot connections, try the next
                    return setImmediate(() => tryConnect());
                }

                let lockKey = [mx.host, delivery.zoneAddress.address].join(":");
                this.sendCommand({ cmd: 'LOCKMTA', pid: process.pid, key: lockKey}, () => false);
